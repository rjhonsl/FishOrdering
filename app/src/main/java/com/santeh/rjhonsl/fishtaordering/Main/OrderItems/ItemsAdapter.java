...
public class ItemsAdapter extends RecyclerView.Adapter<ItemsAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        holder.txtItem.setText(itemList.get(holder.getAdapterPosition()).getOrder_description());
        holder.txtQuantity.setText(itemList.get(holder.getAdapterPosition()).getOrder_qty()+" "+itemList.get(holder.getAdapterPosition()).getOrder_unit());
                ...
                popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                    public boolean onMenuItemClick(MenuItem item) {
                        if (item.getItemId() == R.id.itmDelete) {
                            ...
                            String units = itemList.get(holder.getAdapterPosition()).getItem_units();
                            final String[] pax = units.split(",");

                            final Dialog d = Helper.dialogBox.numberAndPAXpicker(activity1, itemList.get(holder.getAdapterPosition()).getOrder_description(), 1, 1000, pax );
                            ...
                            value.setValue(Integer.valueOf(itemList.get(holder.getAdapterPosition()).getOrder_qty()));
                            ...
                            btnOk.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    value.clearFocus();
                                    pax1.clearFocus();
                                    itemList.get(holder.getAdapterPosition()).setOrder_qty(value.getValue()+"");
                                    itemList.get(holder.getAdapterPosition()).setOrder_unit(pax[pax1.getValue()]+"");
